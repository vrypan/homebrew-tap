class Stash < Formula
  desc "A local store for pipeline output and ad hoc file snapshots."
  homepage "https://github.com/vrypan/stash"
  version "0.5.4"
  license "MIT"

  on_macos do
    on_arm do
      url "https://github.com/vrypan/stash/releases/download/v0.5.4/stash-cli-aarch64-apple-darwin.tar.xz"
      sha256 "0ccfb8ed2a97a2ff4af97ff334f2308f12c8915e11c0f4dda642ddbe4c39f5ff"
    end
    on_intel do
      url "https://github.com/vrypan/stash/releases/download/v0.5.4/stash-cli-x86_64-apple-darwin.tar.xz"
      sha256 "64d9cd27dd64f70d3bfc8eeb2e0a362337ddcca8d8133e84c7e7d7d248aedd1a"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/vrypan/stash/releases/download/v0.5.4/stash-cli-aarch64-unknown-linux-gnu.tar.xz"
      sha256 "32fc0429d0e70cdfdd02d54ceecf8516ca7dec41117a89e5d81c4ce70c76fbfe"
    end
    on_intel do
      url "https://github.com/vrypan/stash/releases/download/v0.5.4/stash-cli-x86_64-unknown-linux-gnu.tar.xz"
      sha256 "2ac31d3a3e89a3d55bcde5a4c6fb1953c20e47cf1d97f61669a4df7b7a8c31da"
    end
  end

  def install
    if which("stash")
      installed = Utils.safe_popen_read("stash", "version").strip
      version_text =
        if installed =~ /\Astash (\S+)\z/
          Regexp.last_match(1)
        elsif installed =~ /\A\d+\.\d+\.\d+(?:[-+][^\s]+)?\z/
          installed
        end

      if version_text
        existing_version = Version.new(version_text)
        if existing_version < Version.new("0.5.0")
          odie <<~EOS
            stash #{existing_version} is installed.
            Automatic upgrade from versions older than 0.5.0 is not supported.
            Visit https://github.com/vrypan/stash for more info.
            To force the upgrade: brew uninstall stash first, then install again.
          EOS
        end
      end
    end

    bin.install "stash"
    pkgshare.install "scripts" if Dir.exist?("scripts")
    (bash_completion/"stash").write Utils.safe_popen_read(
      bin/"stash", "completion", "bash"
    )
    (zsh_completion/"_stash").write Utils.safe_popen_read(
      bin/"stash", "completion", "zsh"
    )
    (fish_completion/"stash.fish").write Utils.safe_popen_read(
      bin/"stash", "completion", "fish"
    )
  end

  test do
    system "#{bin}/stash", "version"
  end
end
